...
def run_job_cluster(executable, arguments = [], stdin = [],
    ...
    if os.environ.get('USER', '') == 'gda':
        ...
        queue = 'fast'                      # Modified by Y.Yamada to adjust for NSLS-II environment

# Commented out by Y.Yamada to adjust for NSLS-II environment
#    if ncpu > 1:
#        qsub_output = run_job(
...
# Added by Y.Yamada to adjust for NSLS-II environment
    qsub_output = run_job(
        'qsub', timeout_tokens + ['-V', '-cwd', '-q', queue,
                                  'FEP_%s.sh' % rs], [], working_directory)
